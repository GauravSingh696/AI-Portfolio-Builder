...
`
  ...
  const handlePublish = () => {
    ...
    try {
      const route = `/portfolios/${publishSubdomain.trim()}`
      // Save HTML under a route-specific key for preview
      localStorage.setItem(`portfolioHTML:${publishSubdomain.trim()}`, portfolioHTML)
      setPublishedUrl(route)
      setPublishSuccess(true)
      toast.success("Portfolio published for preview.")
      setShowSaveDialog(false)
      setPublishSubdomain("")
      // Redirect to publish success page with sharable path
      router.push(`/publish-success?url=${encodeURIComponent(route)}`)
    } catch (error) {
    ...
    <SidebarProvider>
      <div className="flex h-screen w-full flex-col overflow-hidden bg-background">
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Main content area only – sidebar customization removed */}
          ...
          {!publishSuccess ? (
            <>
              <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                  <Label htmlFor="subdomain">Choose your preview route</Label>
                  <div className="flex items-center gap-2">
                    <Input
                      id="subdomain"
                  ...
                  <p className="text-xs text-muted-foreground">
                    This will be the path for your preview link, e.g. /portfolios/your-portfolio
                  </p>
                </div>
              </div>
            ...
            <div className="py-6 flex flex-col items-center justify-center">
              ...
              <p className="mt-2 text-center text-sm text-muted-foreground">
                Your portfolio is now available for preview at{" "}
                <code className="font-mono">{publishedUrl}</code>
              </p>
              <Button
                className="mt-4"
                variant="outline"
